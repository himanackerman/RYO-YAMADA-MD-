...
let handler = async (m, { text, conn, usedPrefix, command }) => {
  ...
  try {
    const imageUrl = "https://i.ibb.co/67pkgwdy/elaina-md.jpg"
    ...
    const boardX = img.width * 0.15
    const boardY = img.height * 0.66
    const boardW = img.width * 0.70
    const boardH = img.height * 0.25
...
handler.help = ["bratryo"]
handler.tags = ["maker"]
handler.command = /^bratryo$/i
handler.limit = true
