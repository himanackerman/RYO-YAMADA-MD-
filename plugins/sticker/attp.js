...
async function create_attp(text) {
  ...
  return new Promise((resolve, reject) => {
    const output_webp = path.join(process.cwd(), `attp_${Date.now()}.webp`);
    ...
    exec(ffmpeg_cmd, (error) => {
      ...
      if (fs.existsSync(output_webp)) fs.unlinkSync(output_webp);
      resolve(buffna);
    });
  });
}
...
async function create_ttp(text) {
  ...
  const buffer = canvas.toBuffer('image/png');
  const output_webp = path.join(process.cwd(), `ttp_${Date.now()}.webp`);
...
let handler = async (m, { conn, text, command }) => {
  ...
  try {
    let rawBuffer;
    if (command === 'ttp') {
      rawBuffer = await create_ttp(text);
    } else if (command === 'attp') {
      rawBuffer = await create_attp(text);
    } else {
      return m.reply('Perintah tidak valid.');
    }

    // Menambahkan Watermark / Exif via lib/sticker.js
    const stickerBuffer = await addExif(rawBuffer, 'ʀyᴏ yᴀᴍᴀᴅᴀ - ᴍᴅ', 'ʙy ʜɪʟᴍᴀɴ');
