...
const handler = async (m, { conn, usedPrefix, command, args }) => {
  ...
  user.polisi = user.polisi || { Uang: 0, Pencuri_Tertangkap: 0, Waktu_Tertangkap: 0, Kaca_Pembesar: 0, Level: 1, State: STATES.IDLE };
  const player = user.polisi;
  ...
  if (command === "polisi") {
    ...
    if (subCommand === "cari") {
      ...
      const leaderboard = Object.entries(global.db.data.users)
        .filter(([, userData]) => userData.polisi)
        .map(([playerId, userData]) => ({ id: playerId, Pencuri_Tertangkap: userData.polisi.Pencuri_Tertangkap || 0 }))
