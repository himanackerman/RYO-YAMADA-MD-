...
const {
    ...
    MessageType,
    Mimetype
} = Baileys.default?.default ? Baileys.default : Baileys

const _makeWaSocket = Baileys.default?.default ? Baileys.default.default : (Baileys.default || Baileys)
...
export function smsg(conn, m, hasParent) {
    ...
    // simpan dulu field Alt (dari fitur LID) SEBELUM dikonversi via protobuf.
    // skema protobuf yang dipaket library ini kadang belum kenal field baru
    // kayak participantAlt/remoteJidAlt, jadi field ini bisa kebuang diam-diam
    // pas fromObject() kalau gak diselamatin manual dulu.
    const _rawParticipantAlt = m?.key?.participantAlt
    const _rawRemoteJidAlt = m?.key?.remoteJidAlt

    m = M.fromObject(m)

    if (m.key) {
        if (_rawParticipantAlt && !m.key.participantAlt) m.key.participantAlt = _rawParticipantAlt
        if (_rawRemoteJidAlt && !m.key.remoteJidAlt) m.key.remoteJidAlt = _rawRemoteJidAlt
    }
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                ...
                // WhatsApp sekarang bisa ngasih @lid (Local ID, buat privasi) alih-alih
                // nomor asli. Kalau kena @lid, coba ambil versi nomor asli (PN) dari
                // field Alt yang disediakan Baileys (participantAlt buat grup,
                // remoteJidAlt buat chat pribadi).
                if (typeof raw === 'string' && raw.endsWith('@lid')) {
                    const alt = this.key?.participantAlt || this.key?.remoteJidAlt
                    if (alt) raw = alt
                }

                return this.conn?.decodeJid(raw)
            },
            enumerable: true
        },
        fromMe: {
